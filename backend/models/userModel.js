...
// Prevent Mongoose from auto-building old indexes
...
userSchema.index({ "moodTracking.enabled": 1, "moodTracking.aiAnalysisConsent": 1 });
userSchema.index({ joinedDate: -1, "moodTracking.enabled": 1 });

const userModel = mongoose.models.user || mongoose.model("user", userSchema);

// Drop the problematic username index safely
userModel.collection.dropIndex('username_1').catch(err => {
